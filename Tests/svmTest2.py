...
tuned_parameters = [{'kernel': ['rbf'], 'gamma': [0],'C': [0.1]},{'kernel': ['rbf'], 'gamma': [0],'C': [1]}]
...
def run(X, y,XTest,yTest, foldsNum,path,inds, jobsNum=6, calcProbs=True):
    ...
    for tuned_param in tuned_parameters:
        params = []
        params.append((X, y, XTest, yTest, tuned_param, -1, calcProbs))


        if (jobsNum == 1):
            mapResults = [calc_full_scores(p) for p in params]  # For debugging
        else:
            mapResults = utils.parmap(calc_full_scores, params, jobsNum)
...
def calc_full_scores(p):
    t = time.time()
    X, y, XTest, YTest, tuned_param, fold_num, calcProbs = p
...
def go():
    ...
    path='/home/ohadfel/Copy/Baus/Code/matlab/inds.mat'
    ...
    run(X, y,Xtest, Ytest, foldsNum, path, inds)
    print('finish!')
